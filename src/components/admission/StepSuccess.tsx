...
export function StepSuccess({ planType, onFinish }: Props) {
  ...
  // Generate mock dates based on SLA
  const generateDates = () => {
    const dates = [];
    ...
    for (let i = 0; i < 5; i++) {
      ...
      // Skip weekends for social/premium
    ...
    return (
      ...
      >
        ...
        <p className="text-lg text-slate-600 max-w-md mx-auto">
          Tu pago ha sido procesado exitosamente y tu caso ya está en revisión por nuestros especialistas.
        </p>
        
        <div className="bg-slate-50 rounded-2xl p-6 max-w-md mx-auto text-left space-y-4 border border-slate-200">
          <div className="flex items-center gap-3 text-slate-700">
            <Video className="w-5 h-5 text-blue-600" />
            <span>Enlace de Google Meet generado</span>
          </div>
          <div className="flex items-center gap-3 text-slate-700">
            <Mail className="w-5 h-5 text-blue-600" />
            <span>Invitación de calendario (iCal) enviada a tu correo</span>
          </div>
        </div>
  ...
  return (
    <div className="space-y-8">
      <div className="text-center mb-8">
        ...
        <p className="text-slate-500 mt-2">Por favor, selecciona el horario para tu videollamada.</p>
      </div>
